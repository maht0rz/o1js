...
if (globalThis.__o1js_backend_preference === undefined) {
    let env = getBackendFromEnv();
    if (env !== undefined) {
        globalThis.__o1js_backend_preference = env;
    }
}
/**
...
function setBackend(backend) {
    if (locked) {
        throw new Error('setBackend() must be called before initializeBindings(). ' +
    ...
    if (backend === 'native' && typeof window !== 'undefined') {
        console.error("No-op: `setBackend()`:The native backend is not available in the browser. Only 'wasm' is supported. Falling back to 'wasm' backend.");
        globalThis.__o1js_backend_preference = 'wasm';
    }
    globalThis.__o1js_backend_preference = backend;
}
/**
...
function getBackendPreference() {
    return globalThis.__o1js_backend_preference ?? 'wasm';
}

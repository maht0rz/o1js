...
function run() {
    ...
    let bEmpty = BerkeleyAU.empty();
    let mEmpty = MesaAU.empty();
    ...
    let payM = mesa.signPayment(V.payment.input, privateKey);
    let payB = berkeley.signPayment(V.payment.input, privateKey);
    ...
    let delM = mesa.signStakeDelegation(V.stakeDelegation.input, privateKey);
    let delB = berkeley.signStakeDelegation(V.stakeDelegation.input, privateKey);
    ...
    let msgM = mesa.signMessage(V.message.input, privateKey);
    let msgB = berkeley.signMessage(V.message.input, privateKey);
    ...
    let fieldsIn = V.fields.input.map((s) => BigInt(s));
    ...
    check('fields: matches @3.0.7 golden', fM.signature === V.fields.output.signature);
    // nullifier is randomized (Scalar.random) - just confirm both eras produce a valid shape
    let nullIn = V.nullifier.input.map((s) => BigInt(s));
    let nB = berkeley.createNullifier(nullIn, privateKey);
    ...
    let cmd1 = clone(V.zkapp1.output.data.zkappCommand);
    let signed1 = signZkappCommand(cmd1, privateKey, network, 'berkeley');
    check('zkapp1: berkeley feePayer signature matches @3.0.7', signed1.feePayer.authorization === V.zkapp1.output.signature);
    check('zkapp1: berkeley signature verifies', verifyZkappCommandSignature(signed1, publicKey, network, 'berkeley'));
    let cmd2 = clone(V.zkapp2.output.data.zkappCommand);
    let signed2 = signZkappCommand(cmd2, privateKey, network, 'berkeley');
    check('zkapp2: berkeley feePayer signature matches @3.0.7', signed2.feePayer.authorization === V.zkapp2.output.signature);
    ...
    let cmd1Mesa = clone(V.zkapp1.output.data.zkappCommand);
    let signed1Mesa = signZkappCommand(cmd1Mesa, privateKey, network, 'mesa');
    check('positive delta: berkeley and mesa produce different signatures for the same zkApp command', signed1Mesa.feePayer.authorization !== signed1.feePayer.authorization);
    ...
    let mesaShaped = clone(V.zkapp1.output.data.zkappCommand);
    ...
    let okShaped = clone(V.zkapp1.output.data.zkappCommand);
    ...
    let mesaPermissive = clone(V.zkapp1.output.data.zkappCommand);
...
run();
//# sourceMappingURL=berkeley.unit-test.js.map